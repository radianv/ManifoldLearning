...
# archivo_csv = 'DATA/denhaag.csv'
archivo_csv = sys.argv[1]

####Leemos el archivo csv que se pasa la ubicacion desde linea de comandos
datos = pd.read_csv(archivo_csv, header=None)
## SHAROP: Comente esta linea para que pudiera leer el archivo de migracion de poblacion.  
### TODO: Hay que hacer cambios para que acepte parametros para leer el dataset.
datosa = datos   #[list(datos.columns)[4:]]

####Obtenemos la primera componente principal
pca = decomposition.PCA()
X_pca = pca.fit_transform(datosa)
...
for key in intervalo_indices.keys():
    ...
    indices = range(len(datosa.ix[intervalo_indices[key]]))
    ...
    nuevas_observaciones = copy.copy(datosa.ix[intervalo_indices[key]].values)
    ...
    while len(nuevas_observaciones) > 0:
        ...
        if len(nuevas_observaciones) > vecinos:
            ...
            nuevas_observaciones = [nuevas_observaciones[idx] for idx in range(len(nuevas_observaciones)) if
                                    idx not in grupo]
...
for nodo in range(len(intervalos_clusters.keys())):
    ...
    nodos.append({'group': nodo,
                  'name': '''<tspan x="10" y="45"> Nodo {0} </tspan>
                    <tspan x="10" y="70">Elementos{1}</tspan> '''.format(nodo, len(intervalos_clusters[nombre])),
